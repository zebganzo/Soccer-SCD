...
--                 new_target := Coordinate'(target.coord_x-2,target.coord_y+2);
   ...
   task body Player is
      ...
      ball_x          : Integer := middle_field_x;
      ball_y          : Integer := middle_field_y;
...
--                          String'Write(Stream(output), To_String(assert_event) & Ada.Characters.Latin_1.CR);
			...
			elsif subbed and player_team = Team_Two and (player_position.coord_x = id + team_two_offset and player_position.coord_y = 0)  then
                           subbed := False;
                           assert_sub := To_Unbounded_String("substitution(in)");
                        elsif subbed then
